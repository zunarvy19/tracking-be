...
export const reportService = {
    async getSummary(userId, period = "this_month") {
        ...
        const income = parseFloat(result.find((r) => r.type === "income")?.total || "0");
        const expense = parseFloat(result.find((r) => r.type === "expense")?.total || "0");
        ...
        const prevResult = await db
            ...
            .from(transactions)
            .where(and(eq(transactions.userId, userId), gte(transactions.date, prevStart.toISOString().split("T")[0]), lte(transactions.date, prevEnd.toISOString().split("T")[0])))
            .groupBy(transactions.type);
        const prevIncome = parseFloat(prevResult.find((r) => r.type === "income")?.total || "0");
        const prevExpense = parseFloat(prevResult.find((r) => r.type === "expense")?.total || "0");
    ...
    async getExpenseBreakdown(userId, period = "this_month") {
        const { start, end } = getPeriodRange(period);
        const result = await db
            .select({
        ...
        const totalExpense = result.reduce((sum, r) => sum + parseFloat(r.total), 0);
        return result.map((r) => ({
    ...
    async getMonthlyComparison(userId, months = 5) {
        const now = new Date();
        const result = [];
        for (let i = months - 1; i >= 0; i--) {
            ...
            const totals = await db
                ...
                .from(transactions)
                .where(and(eq(transactions.userId, userId), gte(transactions.date, monthStart.toISOString().split("T")[0]), lte(transactions.date, monthEnd.toISOString().split("T")[0])))
                .groupBy(transactions.type);
            result.push({
                month: monthName,
    ...
    async getLargeTransactions(userId, period = "this_month", limit = 5) {
        const { start, end } = getPeriodRange(period);
        const result = await db
            .select({
            id: transactions.id,

...
function getCurrentPeriodRange(period: "weekly" | "monthly") {
  const now = new Date();
  let start: Date;
  let end: Date;
...
export const budgetService = {
  async list(userId: string) {
    ...
    const result = await Promise.all(
      budgetList.map(async (budget) => {
        ...
        const spentResult = await db
          ...
          .where(
            and(
              ...
              eq(transactions.categoryId, budget.category!.id),
        ...
        if (percentage >= 100) statusLabel = "Alert";
        else if (percentage >= 90) statusLabel = "Limit Reached";
        else if (percentage >= 75) statusLabel = "Warning";
        else if (percentage >= 50) statusLabel = "On Track";
  ...
  async update(userId: string, id: string, data: Partial<CreateBudgetDTO>) {
    const result = await db
      .update(budgets)
      .set({
        ...data,
